...
export class FilesComponent implements OnInit {
  constructor(
    ...
    private _datingService: DatingService,
    private _doct: DocxtemplaterService
  ) { }
  ...
  dataSourceControls: MatTableDataSource<
    ControlRiesgo[]
  ...
  generar() {
    ...
    this._datingService.GuardarEvaluacion(model).subscribe(
      data => {
        this.download();
      },
      error => {
  ...
  download() {
    this._datingService.ExportDoc().subscribe(
      data => {
        console.log(data);
  ...
  evaluar(row: any) {
    ...
    for (var i = 0; i < this.riesgos.length; i++) {
      if (
        this.riesgos[i].identificacionid ==
  ...
  getControls() {
    this._datingService.getListControls().subscribe(
      data => {
        console.log(data);
  ...
  getCaracteristicasPorFactor(IdFactor: number) {
    this._datingService.getCaracteristicasPorFactor(IdFactor).subscribe(
      data => {
        if (IdFactor == 1) {
    ...
    if (IsAdd) {
      ...
      for (var i = 0; i < this.riesgos.length; i++) {
        if (this.riesgos[i].identificacionid ==this.ActualRiesgoSeleccionado.identificacionid
          ...
          if (this.riesgos[i].controles.length > 1) {
            return;
          }
        }
      }
      this.RiesgoResidual = this.calcularIdentidad4(this.RiesgoResidual);
    }
  }
  ...
  setRiesgoResidualInicial(){
    ...
    for (var i = 0; i < this.riesgos.length; i++) {
      if (this.riesgos[i].identificacionid ==this.ActualRiesgoSeleccionado.identificacionid) {
        ...
        {
          this.RiesgoResidual = this.calcularIdentidad3(this.RiesgoInherente);
        }
      }
    }
